...
\title{Subword Tokenization Efficiency for Bengali Language Modeling}
...
\begin{abstract}
We present a systematic evaluation of subword tokenization strategies for Bengali, comparing 14 tokenizers across dedicated Bengali models, multilingual LLMs, and newly trained variants.
Our evaluation on 3,000 Bengali documents reveals that Bengali-dedicated tokenizers (1.47--1.75 tokens/word) are up to 9.3$\times$ more efficient than most multilingual alternatives (7.32--13.69 tokens/word), though BLOOM (1.75) is a notable exception among multilingual models.
We find that (1) a single missing Unicode combining character (U+09BC Bengali Nukta) in tokenizer training data is associated with a byte-fallback rate increase from 2\% to 20\%, with quantitative analysis showing this character accounts for 89.8\% of all byte-fallback tokens, and (2) Bengali-dedicated tokenizers that memorize whole words achieve superior efficiency but sacrifice morphological decomposition that may benefit generative model generalization.
We release our evaluation framework, trained tokenizers, and benchmark data to support future Bengali NLP research.
\end{abstract}
